-- ============================================================================
-- RESET COMPLETO — dashboard-publibe
-- ============================================================================
-- Confirmado por Diego: los datos actuales de las tablas son SOLO DE PRUEBA.
-- Este script es un solo copy-paste que deja la base lista y alineada con el
-- código:
--   1. Borra las tablas viejas (creadas antes con otra estructura — la causa
--      de los errores 400 en cascada y del seeding fallido de usuarios).
--   2. Limpia las banderas de "ya se sembraron datos de ejemplo" para que la
--      app vuelva a sembrar todo limpio en la próxima carga.
--   3. Recrea todo con la estructura que el código espera (id de texto,
--      columnas empresa + data jsonb, email en users), con RLS y Realtime.
--
-- Cómo usarlo: Supabase -> SQL Editor -> New query -> pegar TODO este archivo
-- -> Run. Después recargá la app (Ctrl+Shift+R): Diego y Ariana aparecen en
-- el login sembrados automáticamente, y las demás tablas cargan sin errores.
-- ============================================================================

-- 1) Borrar las tablas con estructura vieja
...
-- 2) Limpiar banderas de siembra (viven en kv_store, que se conserva; si
--    kv_store también tenía estructura vieja, el drop de abajo la alinea)
drop table if exists kv_store cascade;

-- 3) Recrear todo con la estructura correcta (contenido de schema.sql)
-- ============================================================================
